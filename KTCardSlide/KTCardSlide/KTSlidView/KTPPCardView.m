...
@interface KTPPCardView ()<KTPPCardViewCellDelagate>
...
@property (nonatomic, strong) NSArray<__kindof KTPPCardViewCell *> *visibleCells;//!< 当前可视cells
@property (nonatomic, strong) NSMutableArray<__kindof KTPPCardViewCell *> *reusableCells;//!< 重用卡片数组
...
- (void)createCardViewCellWithIndex:(NSInteger)index {
    ...
    KTPPCardViewCell * cell = [self.dataSource cardView:self cellForRowAtIndex:index];
...
- (void)updateLayoutVisibleCellsWithAnimated:(BOOL)animated {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        // 获取当前要显示的的cells
        KTPPCardViewCell *cell = self.visibleCells[i];
...
- (void)updateConstraintsCell:(KTPPCardViewCell*)cell transform:(CGAffineTransform)transform {
...
/** 可视cells */
- (NSArray<KTPPCardViewCell *> *)visibleCells {
...
- (__kindof KTPPCardViewCell *)dequeueReusableCellWithIdentifier:(NSString *)identifier {
    for (KTPPCardViewCell *cell in self.reusableCells) {
    ...
    if (self.nib) {
        KTPPCardViewCell *cell = [[self.nib instantiateWithOwner:nil options:nil] lastObject];
        cell.reuseIdentifier = identifier;
        
        return cell;
    } else if (self.cellClass) { // 注册class
        KTPPCardViewCell *cell = [[self.cellClass alloc] initWithReuseIdentifier:identifier];
        cell.reuseIdentifier = identifier;
...
- (nullable __kindof KTPPCardViewCell *)cellForRowAtIndex:(NSInteger)index {
    ...
    KTPPCardViewCell *cell = [self.visibleCells objectAtIndex:visibleIndex];
    return cell;
}
...
- (void)removeTopCardViewFromSwipe:(KTPPCardViewCellSwipeDirection)direction {
    ...
    KTPPCardViewCell * topcell = [self.visibleCells lastObject];
